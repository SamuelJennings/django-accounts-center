...
| Login form + "Forgot password?" | `not SOCIALACCOUNT_ONLY` | `<c-form>` + `<c-form.crispy />` + `<c-button>` + `<c-entrance.text>` |
...
| Sign-up cross-link | `signup_url` is truthy | `<c-entrance.text>` with blocktrans |
