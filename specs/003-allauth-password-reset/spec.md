...
- **FR-001**: The addon MUST provide an override for `account/password_reset.html` that renders: the `already_logged_in` snippet when `user.is_authenticated` is true; a description paragraph and a trailing "contact us" paragraph rendered via `<c-text>` (description with `center` modifier, contact-us with `small` modifier); and an email-input form using `<c-form>` with a "Send email" submit button (inside `<c-group>`, with `icon="send"` and `variant="primary"`), including `{{ redirect_field }}` as a hidden input inside the form body — all within a `<c-entrance>` shell.
- **FR-002**: The addon MUST provide an override for `account/password_reset_done.html` that renders: the `already_logged_in` snippet when `user.is_authenticated` is true; and a "check your spam folder" informational paragraph rendered via `<c-text center>` — within a `<c-entrance>` shell. No form or button is present.
- **FR-003**: The addon MUST provide an override for `account/password_reset_from_key.html` that renders both branches within a `<c-entrance>` shell: (a) the valid-token branch renders the new-password form with `{{ redirect_field }}` as a hidden input, a "Confirm" submit button (inside `<c-group>`, with `icon="submit"` and `variant="primary"`), and a secondary "Cancel" button (`icon="x-circle"`) that signs the user out and redirects to the login page; (b) the invalid-token branch renders an explanatory `<c-text>` block containing an inline link to request a new reset.
...
- **FR-005**: The addon MUST provide an override for `account/password_reset_from_key_done.html` that renders a "Your password is now changed." confirmation via `<c-text center>` within a `<c-entrance>` shell. No button, link, or further action is present.
