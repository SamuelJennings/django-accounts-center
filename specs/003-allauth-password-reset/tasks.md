...
- [X] T002 [US1] Write `dac/addons/allauth/templates/account/password_reset.html` — full Cotton rewrite of the existing `{% element %}`-based placeholder. Template must:
      ...
      - `{% block content %}` — ordered sections per contracts/template-context.md:
          ...
          3. `{% url 'account_reset_password' as reset_url %}` → `<c-form method="post" action="{{ reset_url }}">{% csrf_token %}<c-form.render form=form />{{ redirect_field }}<c-group><c-button text="Send email" icon="send" size="lg" type="submit" variant="primary" /></c-group></c-form>`
