...
**Propagated**: 2026-05-08 — Updated T003 (entrance.html now delegates to `<c-entrance>`), T005 (signup.html is content-only; no card/logo markup; uses `<c-group>`; no `{% if form.non_field_errors %}`), and T011 (provider_list uses Bootstrap Icon `<a>` tags instead of `<c-button>`). Added Phase 2b for Cotton entrance component creation.
...
- [X] T013 [US3] Write `dac/addons/allauth/templates/socialaccount/signup.html` — `<c-card>` with provider name + site name in header, `<c-form>` POSTing to `socialaccount_signup`, `<c-form.render />`, `{{ redirect_field }}`, submit `<c-button>`
...
- [X] T015 [US3] Playwright MCP verify — social enabled config: Google button renders above `<c-divider>` with "or" label; social disabled config: no buttons and no divider; `SOCIALACCOUNT_ONLY=True`: password form is absent
