...
- **FR-005**: The signup form MUST display per-field validation errors inline, immediately below the offending field, without losing values in other fields. Non-field (form-level) errors MUST be displayed as a danger alert above the fields; this is handled automatically by `<c-form.render>` — page templates MUST NOT duplicate this logic with their own `{% if form.non_field_errors %}` block.
...
- **`<c-entrance>` Component** (`dac/templates/cotton/entrance/index.html`): The entrance page shell component. Accepts a `title` slot (rendered as an `<h4>` inside the card header) and responsive-width attrs forwarded to ``. Renders `<c-entrance.background>` → container → responsive column → card → `<c-entrance.logo>` → title → `{{ slot }}`.
