...
- Q: Should the Sessions page render a visible heading in `{% block breadcrumbs %}`? → A: Yes — render `{% trans "Sessions" %}` in `{% block title %}`. Research (Decision 6) confirmed that `{% block breadcrumbs %}` wraps the breadcrumbs toolbar region, not a heading slot; the heading appears via `{% block title %}` inside `<c-mvp.toolbar>`, consistent with all other DAC management pages.
