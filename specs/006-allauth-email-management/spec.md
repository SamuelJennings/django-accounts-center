...
- The Cotton components used by `email.html` and `email_change.html` (`c-badge`, `c-dropdown`, `c-dropdown.item`, `c-card`, `c-list-group`, `c-list-group.item`, `c-form.card`, `c-form`, `c-button`, `c-group`) are available through `django-mvp`, `django-cotton-bs5`, or existing DAC custom components. `c-form.card` is the preferred form wrapper for management pages; `c-form` is the fallback when `c-form.card` lacks the needed functionality.
