...
- Q: Should `email_change.html` keep `<c-form>` or switch to nested `<c-form>` + `<c-card>`? → A: Keep `<c-form>` — it is the preferred wrapper for management-page forms. Nested `<c-form>` + `<c-card>` is only acceptable when `<c-form>` cannot provide the required functionality.
...
- The Cotton components used by `email.html` and `email_change.html` (`c-badge`, `c-dropdown`, `c-dropdown.item`, `c-card`, `c-list`, `c-list.item`, `c-form`, `c-form`, `c-button`, `c-group`) are available through `django-mvp`, `django-cotton-bs5`, or existing DAC custom components. `c-form` is the preferred form wrapper for management pages; `c-form` is the fallback when `c-form` lacks the needed functionality.
