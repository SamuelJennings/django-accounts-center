...
4. **Given** `mfa/totp/activate_form.html`, **When** rendered with form and QR code context, **Then** the QR code image, the authenticator secret field, the TOTP code input field, and an "Activate" submit button are all present.
...
1. **Given** a user with registered security keys, **When** `mfa/webauthn/authenticator_list.html` is rendered, **Then** each key appears as a table row showing its name, type badge, and "Edit"/"Remove" action links.
...
- What happens when `unused_code_count=0` on `recovery_codes/generate.html`? The invalidation warning is omitted; the "Generate" button renders with a non-danger style (no `tags="danger"` on the original button element).
...
- **FR-002**: `mfa/index.html` MUST be fully rewritten as a clean Cotton template. It MUST override `{% block title %}` with `{% trans "Two-Factor Authentication" %}`, append a "Two-Factor Authentication" item to `{% block page.breadcrumbs %}`, and place all content inside `{% block page.content %}` (not `{% block content %}`). The three method panels (TOTP, Security Keys, Recovery Codes) MUST each be rendered as an individual `<c-card title="...">` — the `title` attribute supplies the panel heading, and action links (Activate, Deactivate, Manage, Add, View, Download, Generate) MUST be placed in `<c-slot name="actions">` so they render inside the card-header toolbar via `<c-action-group>`. Each panel MUST only render when its method appears in `MFA_SUPPORTED_TYPES`. The full rewrite is warranted because the existing template uses `{% block content %}` and allauth `{% element %}` tags throughout.

- **FR-003**: `mfa/totp/activate_form.html` MUST be fully rewritten as a clean Cotton template. It MUST use `<c-form.card>` POSTing to `{% url 'mfa_activate_totp' %}`, matching the established pattern in `account/password_change.html`. The QR code image MUST be rendered as a standard `<img>` tag using `totp_svg_data_uri`. The authenticator secret MUST be rendered as a read-only text field. The TOTP code input MUST be rendered via `<c-form.fields form=form />` or equivalent inside the default slot. The submit button MUST be placed in `<c-slot name="actions">` as `<c-button type="submit">{% trans "Activate" %}</c-button>`.

- **FR-004**: `mfa/totp/deactivate_form.html` MUST be fully rewritten as a clean Cotton template. It MUST use `<c-form.card>` POSTing to `{% url 'mfa_deactivate_totp' %}`. The confirmation text MUST be rendered in a standard `<p>` tag inside the default slot. The submit button MUST be placed in `<c-slot name="actions">` as `<c-button variant="danger" type="submit">{% trans "Deactivate" %}</c-button>`.
...
- **FR-006**: `mfa/recovery_codes/generate.html` MUST be fully rewritten as a clean Cotton template extending `mfa/recovery_codes/base.html`. The confirmation text block MUST conditionally include the invalidation warning when `unused_code_count > 0`. The form MUST use `<c-form.card>` POSTing to `{% url 'mfa_generate_recovery_codes' %}`. The submit button MUST be placed in `<c-slot name="actions">` as `<c-button variant="danger">` when `unused_code_count > 0`, and without a `variant` attribute (default style) otherwise.

- **FR-007**: `mfa/webauthn/authenticator_list.html` MUST be fully rewritten as a clean Cotton template extending `mfa/webauthn/base.html`. All content MUST be placed inside `{% block page.content %}` (not `{% block content %}`). When authenticators are present, they MUST be rendered in a Bootstrap table (`<table class="table">`) with columns for key name/type badge and actions. Type badges MUST use `<c-badge>` with variant `primary` for passkeys, `secondary` for security keys, and `warning` for unspecified. Edit and Remove action links MUST use `<c-button>` components. The "Add" button MUST be placed in `<c-slot name="actions">` in the card-header toolbar using `<c-button>`. When no authenticators are registered, an informational message paragraph MUST be shown.

- **FR-008**: `mfa/webauthn/add_form.html` MUST be rewritten replacing `{% element %}` / `{% slot %}` / `{% endelement %}` tags with Cotton equivalents. All content MUST be placed inside `{% block page.content %}` (not `{% block content %}`). The `{% include "mfa/webauthn/snippets/scripts.html" %}`, `{{ js_data|json_script:"js_data" }}` script, and `<script data-allauth-onload="allauth.webauthn.forms.addForm">` block MUST be preserved exactly. The WebAuthn button MUST retain `id="mfa_webauthn_add"`.

- **FR-009**: `mfa/webauthn/edit_form.html` MUST be rewritten replacing `{% element %}` / `{% slot %}` / `{% endelement %}` tags with Cotton equivalents. All content MUST be placed inside `{% block page.content %}` (not `{% block content %}`). The form MUST POST to `{% url 'mfa_edit_webauthn' %}` and include the key name field via `<c-form.fields form=form />` or equivalent. The submit button MUST use `<c-button type="submit">`.

- **FR-010**: `mfa/webauthn/authenticator_confirm_delete.html` MUST be rewritten replacing `{% element %}` / `{% slot %}` / `{% endelement %}` tags with Cotton equivalents. All content MUST be placed inside `{% block page.content %}` (not `{% block content %}`). The form MUST POST to `{% url 'mfa_remove_webauthn' pk=authenticator.pk %}`. The submit button MUST use `<c-button variant="danger" type="submit">{% trans "Remove" %}</c-button>`.
