...
nothing to. After this feature the developer attaches a per-request answer to each entry they
...
1. **Given** an integration that contributes a menu entry with a per-request answer, **When** the
...
4. **Given** a menu entry contributed without a per-request answer, **When** the Account Center
...
- A menu entry contributed with no per-request answer stays visible whenever its integration is
...
- **FR-002**: The declaration MUST be evaluated for each request rather than once when the
...
- **FR-005**: A menu entry contributed without a declaration MUST remain visible whenever its
...
- **Menu entry**: One item in the Account Center menu, contributed by the core package or by an
  ...
  - an icon
  - a destination
  - after this feature, an optional answer to whether it applies to the current person
...
- **SC-004**: An integration written before this feature, contributing entries with no
  declaration, produces the same menu it produced before.
...
- Declaring an answer is optional. An integration that says nothing keeps today's behaviour, and
