...
- **FR-001**: The addon MUST provide an override for `account/verification_sent.html` that renders an informational message (no form, no submit button) within a `<c-entrance>` shell, using `<c-text center>` for descriptive paragraphs, consistent with the visual style of the signup and login pages.
- **FR-002**: The addon MUST provide an override for `account/email_confirm.html` that handles both branches within a `<c-entrance>` shell: (a) the valid-key branch (`can_confirm = True`) renders a confirm form using `<c-form>` and `<c-button>` with a primary submit button labelled `"Confirm"` with `icon="check-circle"` (inside `<c-group>`, `variant="primary"`); (b) the invalid-key branch renders an explanatory `<c-text>` block with guidance on next steps, and no form or submit button. Integration tests for the confirm button MUST assert that button text is non-empty and that an icon element is rendered — they MUST NOT assert the specific label string or icon name, as these may change independently of the template structure.
...
- **FR-004**: The addon MUST provide an override for `account/confirm_email_verification_code.html` that uses the `<c-allauth.confirm-code>` Cotton component (from `cotton/allauth/confirm_code.html`) and MUST explicitly declare the `resend-supported` attribute to enable the resend button and form. The template renders correctly in contexts where the code-based URL is not registered via fail-silent `{% url ... as u %}{{ u }}` patterns for `action` and `resend-url` attributes.
- **FR-005**: The addon MUST provide an override for `account/account_inactive.html` that uses the `<c-entrance>` Cotton component shell rather than extending `allauth/layouts/entrance.html` directly. The page content is a brief explanatory message rendered via `<c-text center>`, with no form or interactive controls.
...
- **FR-008**: All four template overrides MUST conform to the component usage patterns documented in `contracts/component-interface.md`. Specifically: primary user-facing forms MUST use `<c-form>` (not raw `<form>`); button groups MUST use `<c-button>` and `<c-group>`; the `<c-entrance>` shell MUST wrap each page body; descriptive and informational text MUST use `<c-text>` rather than raw `<p>` tags. Any deviation MUST be justified in a PR comment and reflected in an update to the contracts document.
...
- **SC-006**: The code-entry page (`confirm_email_verification_code.html`) and `account/confirm_login_code.html` both use `<c-allauth.confirm-code>` and both declare `resend-supported`. `account/confirm_password_reset_code.html` uses the same component but does NOT declare `resend-supported` (the password reset flow never supports resend). A source inspection of all three files confirms this attribute is present on exactly the two flows whose allauth process class overrides `can_resend`.
...
- Q: Which `<c-text>` modifier should `verification_sent.html` and `account_inactive.html` use? → A: `center` modifier on all `<c-text>` paragraphs on both pages, consistent with `password_reset_done.html` and `password_reset_from_key_done.html` in Spec 003.
...
- `confirm_email_verification_code.html` uses `<c-allauth.confirm-code>` directly (the same component used by `confirm_login_code.html` and `confirm_password_reset_code.html`). The base template `base_confirm_code.html` no longer exists — it was replaced by the `cotton/allauth/confirm_code.html` Cotton component.
- The Cotton components `<c-form>`, `<c-button>`, `<c-entrance>`, and `<c-text>` are already available in the addon and have been validated by the signup, login, and password-reset template implementations.
